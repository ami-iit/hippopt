...
def get_terrain(length: float, width: float, height: float) -> hp_rp.TerrainDescriptor:
    first_step_origin = np.array([1.5 * length, 0.0, 0.0])
    second_step_origin = np.array([2 * length, 0.0, 0.0])

    terrain = hp_rp.SmoothTerrain.step(
        length=2 * length, width=width, height=height, position=first_step_origin
    ) + hp_rp.SmoothTerrain.step(
        length=0.9 * length, width=width, height=height, position=second_step_origin
    )
    terrain.set_name(f"two_steps_{length}_{width}_{height}")
    return terrain
...
if __name__ == "__main__":
    ...
    contact_phases_guess.left = [
        ...
        hp_rp.FootContactPhaseDescriptor(
            transform=liecasadi.SE3.from_translation_and_rotation(
                np.array([step_length, 0.1, 2 * step_height]),
                liecasadi.SO3.Identity(),
            ),
    ...
    contact_phases_guess.right = [
        ...
        hp_rp.FootContactPhaseDescriptor(
            transform=liecasadi.SE3.from_translation_and_rotation(
                np.array([0.5 * step_length, -0.1, step_height]),
                liecasadi.SO3.Identity(),
            ),
            mid_swing_transform=liecasadi.SE3.from_translation_and_rotation(
